...
namespace MovementApp.Fragments
{
    public class CheckOutFragment : SupportFragment
    {
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            View view = inflater.Inflate(Resource.Layout.fragment_check_out, container, false);
            search_asset_box = view.FindViewById<SearchableSpinner>(Resource.Id.et_check_out_asset);
            search_user_box = view.FindViewById<SearchableSpinner>(Resource.Id.et_check_out_user);

            assetRecyclerView = view.FindViewById<RecyclerView>(Resource.Id.rvCheckOutAssets);

            submit_btn = view.FindViewById<TextView>(Resource.Id.out_submit_btn);
            ...
            try
            {
                ...
                if (selected != "___ Select Asset ___")
                {
                    ...
                    using (var conn = new Connect())
                    {
                        ...
                        if (Movement == null)
                        {
                            data_ = new { tag = tag, direction = "out", reason = "check out", condition = "exellent", date = DateTime.UtcNow, site_id = 1 };
                        }
                        else
                        {
                            data_ = new { tag = tag, direction = "out", reason = "check out", condition = "exellent", date = DateTime.UtcNow, movement_id = Movement.Id, site_id = 1 };
                        }
        ...
        private async void LoadMovementAssets()
        {
            ...
            using (var conn = new Connect())
            {
                string json = await conn.GetAsync(Defaults.SAVED_OUT_MOVEMENTS, token.Token);
